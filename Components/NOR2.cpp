...
void NOR2::Draw(Output* pOut)
{
	...
	pOut->DrawNOR2(m_GfxInfo, Component::getIsSelected());
...
//save 
void NOR2::SaveComponent(ofstream& outputFile)
{
	if (getLabel().empty())
		outputFile << _NOR2 << " " << Component::Get_Comp_Id() << " " <<"$"<< " " << m_GfxInfo.x1 << " " << m_GfxInfo.y1 << endl;
	else
		outputFile << _NOR2 << " " << Component::Get_Comp_Id() << " " << getLabel() << " " << m_GfxInfo.x1 << " " << m_GfxInfo.y1 << endl;
}

//Load gate
int* NOR2::LoadComponent(ifstream& inputFile)
{
	int id_;
	string label_;
	inputFile >> id_;
